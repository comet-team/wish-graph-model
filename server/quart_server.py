...
async def get_recommendations(user_token):
    ...
    print(f"sending request for profile of user {user_token}, number tokens: {answer.shape[0]}")
    answer['creator_id'] = answer.index
    return jsonify(answer.to_numpy().tolist())
